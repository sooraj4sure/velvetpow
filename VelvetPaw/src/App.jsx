...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hero-ornament {
    ...
    border: 0.5px solid rgba(160,120,40,0.3); z-index:0;
  }
  .hero-ornament::before { content:''; position:absolute; inset:40px; border-radius:50%; border: 0.5px solid rgba(160,120,40,0.2); }
  .hero-ornament::after { content:''; position:absolute; inset:80px; border-radius:50%; border: 0.5px solid rgba(160,120,40,0.12); }
  ...
  .hero-headline { font-family:'Playfair Display',serif; font-size:clamp(48px,6vw,80px); font-weight:400; line-height:1.1; color:#1c1610; margin-bottom:20px; }
  .hero-headline em { font-style:italic; color:var(--gold); font-weight:500; }
  .hero-subline { font-family:'Cormorant Garamond',serif; font-size:20px; font-weight:300; color:#7a6a4a; line-height:1.7; margin-bottom:48px; font-style:italic; }
  ...
  .hero-stats { position:absolute; right:60px; bottom:60px; z-index:2; display:flex; gap:48px; }
  .hero-stat-num { font-family:'Cormorant Garamond',serif; font-size:40px; font-weight:300; color:#1c1610; line-height:1; }
  .hero-stat-label { font-size:9px; letter-spacing:0.25em; text-transform:uppercase; color:#9a8a68; margin-top:4px; }
  .hero-divider { width:0.5px; background:rgba(160,120,40,0.3); align-self:stretch; }

  /* SECTION COMMONS */
  .section { padding: 100px 60px; }
  .section-eyebrow { font-size:9px; letter-spacing:0.5em; text-transform:uppercase; color:var(--gold); margin-bottom:16px; display:flex; align-items:center; gap:16px; }
  .section-eyebrow::before { content:''; display:block; width:30px; height:0.5px; background:var(--gold); }
  .section-title { font-family:'Playfair Display',serif; font-size:clamp(36px,4vw,54px); font-weight:400; color:var(--ivory); line-height:1.15; margin-bottom:16px; }
  .section-title em { font-style:italic; color:var(--gold); }
  .section-subtitle { font-family:'Cormorant Garamond',serif; font-size:18px; color:var(--text2); font-style:italic; line-height:1.7; max-width:560px; }
  ...
  .personalities-grid { display:grid; grid-template-columns:repeat(5, 1fr); gap:1px; background:rgba(160,120,40,0.15); }
  .personality-card { background:var(--dark2); padding:40px 28px; cursor:pointer; transition:var(--transition); position:relative; overflow:hidden; }
  .personality-card:hover { background:var(--surface); }
  .personality-card::before { content:''; position:absolute; bottom:0; left:0; right:0; height:2px; background:var(--gold); transform:scaleX(0); transition:var(--transition); transform-origin:left; }
  .personality-card:hover::before { transform:scaleX(1); }
  .personality-num { font-family:'Cormorant Garamond',serif; font-size:48px; font-weight:300; color:rgba(196,160,74,0.15); line-height:1; margin-bottom:16px; }
  .personality-name { font-family:'Playfair Display',serif; font-size:17px; color:var(--ivory); margin-bottom:6px; font-weight:400; transition: color 0.3s; }
  .personality-card:hover .personality-name { color: var(--gold); }
  .personality-tagline { font-family:'Cormorant Garamond',serif; font-size:13px; color:var(--text3); font-style:italic; line-height:1.5; }

  /* MEDIA SHOWCASE */
  .media-bg { background:var(--dark2); }
  .media-grid { display:grid; grid-template-columns: 2fr 1fr 1fr; grid-template-rows: 280px 280px; gap:4px; }
  .media-item { position:relative; overflow:hidden; cursor:pointer; background:var(--surface); display:flex; align-items:center; justify-content:center; }
  .media-item.featured { grid-row: span 2; }
  .media-item-overlay { position:absolute; inset:0; background:linear-gradient(to top, rgba(20,15,8,0.85) 0%, transparent 60%); opacity:0; transition:var(--transition); }
  .media-item:hover .media-item-overlay { opacity:1; }
  .media-item-content { position:absolute; bottom:20px; left:20px; right:20px; opacity:0; transform:translateY(8px); transition:var(--transition); }
  .media-item:hover .media-item-content { opacity:1; transform:translateY(0); }
  .media-item-name { font-family:'Cormorant Garamond',serif; font-size:16px; color:#f0e8d8; font-style:italic; }
  .media-item-breed { font-size:10px; letter-spacing:0.2em; text-transform:uppercase; color:var(--gold-light); }

  /* GET FEATURED */
  .featured-bg { background:var(--dark); }
  .featured-split { display:grid; grid-template-columns:1fr 1fr; gap:80px; align-items:center; }
  .featured-visual { position:relative; aspect-ratio:4/5; background:var(--surface2); border:0.5px solid rgba(160,120,40,0.25); display:flex; align-items:center; justify-content:center; flex-direction:column; gap:12px; }
  .featured-visual::before { content:''; position:absolute; inset:-16px; border:0.5px solid rgba(196,160,74,0.1); pointer-events:none; }
  .featured-visual-text { font-size:11px; letter-spacing:0.2em; text-transform:uppercase; color:var(--text3); }
  .upload-zone { border:0.5px dashed rgba(160,120,40,0.3); padding:48px; text-align:center; margin-bottom:20px; cursor:pointer; transition:var(--transition); position:relative; }
  ...
  .upload-chip { font-size:10px; letter-spacing:0.1em; color:var(--ivory2); background:var(--surface); border:0.5px solid rgba(196,160,74,0.2); padding:6px 14px; display:flex; align-items:center; gap:8px; }
  ...
  .form-input { width:100%; background:var(--surface); border:0.5px solid rgba(160,120,40,0.25); color:var(--ivory); font-family:'Jost',sans-serif; font-size:13px; font-weight:300; padding:12px 16px; outline:none; transition:border-color 0.3s; }
  .form-input:focus { border-color:var(--gold); }
  .form-input::placeholder { color:var(--text3); }
  .form-textarea { min-height:100px; resize:vertical; }

  /* FLOATING BUTTON */
  .float-btn { position:fixed; bottom:36px; right:36px; z-index:99; background:var(--gold); color:var(--dark); border:none; font-family:'Jost',sans-serif; font-size:9px; letter-spacing:0.35em; text-transform:uppercase; padding:14px 24px; cursor:pointer; transition:var(--transition); font-weight:500; box-shadow: 0 4px 32px rgba(160,120,40,0.35); }
  .float-btn:hover { background:var(--gold-light); transform:translateY(-2px); box-shadow:0 8px 40px rgba(160,120,40,0.45); }

  /* MODAL */
  .modal-overlay { position:fixed; inset:0; z-index:200; background:rgba(14,13,11,0.92); display:flex; align-items:center; justify-content:center; opacity:0; pointer-events:none; transition:opacity 0.4s; padding:20px; }
  .modal-overlay.open { opacity:1; pointer-events:all; }
  .modal { background:var(--dark2); border:0.5px solid rgba(160,120,40,0.3); max-width:640px; width:100%; max-height:90vh; overflow-y:auto; transform:translateY(24px); transition:transform 0.4s; position:relative; box-shadow: 0 24px 80px rgba(28,22,16,0.15); }
  .modal-overlay.open .modal { transform:translateY(0); }
  .modal-header { padding:36px 40px 24px; border-bottom:0.5px solid rgba(160,120,40,0.15); position:sticky; top:0; background:var(--dark2); z-index:2; }
  ...
  .modal-close { position:absolute; top:24px; right:32px; background:none; border:none; color:var(--text3); font-size:20px; cursor:pointer; transition:color 0.3s; line-height:1; }
  ...
  .q-option { display:flex; align-items:flex-start; gap:14px; padding:14px 18px; cursor:pointer; border:0.5px solid rgba(160,120,40,0.2); transition:var(--transition); position:relative; }
  ...
  .q-indicator { width:16px; height:16px; border:0.5px solid rgba(160,120,40,0.5); flex-shrink:0; margin-top:1px; display:flex; align-items:center; justify-content:center; transition:var(--transition); }
  ...
  .q-check { display:flex; align-items:center; gap:10px; padding:12px 16px; cursor:pointer; border:0.5px solid rgba(160,120,40,0.2); transition:var(--transition); }
  .q-check:hover { border-color:rgba(160,120,40,0.5); }
  .q-check.selected { border-color:var(--gold); background:rgba(160,120,40,0.08); }
  .q-check-box { width:14px; height:14px; border:0.5px solid rgba(160,120,40,0.5); flex-shrink:0; display:flex; align-items:center; justify-content:center; transition:var(--transition); }
  ...
  .pet-cat { padding:16px 18px; border:0.5px solid rgba(160,120,40,0.2); cursor:pointer; transition:var(--transition); display:flex; flex-direction:column; gap:4px; }
...
function JoinModal({ open, onClose }) {
  ...
  // ✅ This is the CORRECT JoinModal handleSubmit — sends JSON questionnaire data
      ...
      <div className="modal" ref={scrollRef}>
        ...
        <div className="modal-body">
          ...
          {submitted ? (
            ...
            <FinishStep petCategory={petCategory} setPetCategory={setPetCategory} earlyAccess={earlyAccess} setEarlyAccess={setEarlyAccess} />
          )}
        </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              {step === 0 && (
                <button className="btn-primary" disabled={!canNextContact}
                  ...
                  onClick={() => { setStep(1); scrollTop(); }}>Continue →</button>
              )}
              {step === 1 && (
              ...
              {step === 4 && (
                <button className="btn-primary" disabled={!petCategory || !earlyAccess || loading}
                  ...
                  onClick={handleSubmit}>{loading ? "Submitting..." : "Join the Circle →"}</button>
              )}
            </div>
          </div>
        )}
    ...
    <div>
      ...
      <div className="form-group">
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
          {["yes", "no"].map((v) => (
            <div key={v} className={`q-option ${contact.multiple === v ? "selected" : ""}`} style={{ flex: 1 }}
...
function FeaturedModal({ open, onClose }) {
  ...
  // ✅ FIX 1: Store raw File objects — no wrapper object
  const handleFiles = (e) => {
  ...
  // ✅ FIX 2: Append raw File (f) directly — not f.file
  const handleSubmit = async () => {
    const fd = new FormData();
    fd.append("petName", petName);
    fd.append("breed", breed);
    fd.append("caption", caption);
    files.forEach((f) => fd.append("media", f)); // f is a raw File object ✅
    try {
        ...
        {!submitted && (
          <div className="modal-footer">
            ...
            <button className="btn-primary" disabled={files.length === 0 || !petName}
              ...
              onClick={handleSubmit}>Submit for Review →</button>
          </div>
        )}
      </div>
    </div>
    ...
    <>
      ...
      <section className="hero">
        ...
        <div className="hero-content">
          ...
          <h1 className="hero-headline">Where <em>devotion</em><br />becomes an art form.</h1>
          <p className="hero-subline">An invitation-only community for those who understand that a pet is not a companion — they are a presence.</p>
          <div className="hero-actions">
            <button className="btn-primary"
              onClick={() => window.open("mailto:velvetpow@gmail.com?subject=Membership%20Request&body=Hello%20VelvetPaw%2C%0A%0AI%20want%20to%20be%20a%20Member.%0A%0AName%3A%20%0ACity%3A%20%0A%0AThank%20you.", "_blank")}>
              Request Membership
            </button>
            <button className="btn-ghost"
              onClick={() => window.open("https://chat.whatsapp.com/ExampleCommunityLinkHere", "_blank")}>
              Explore Community
            </button>
          </div>
        </div>
        <div className="hero-stats">
          <div><div className="hero-stat-num">10</div><div className="hero-stat-label">Pet Archetypes</div></div>
          <div className="hero-divider" />
          <div><div className="hero-stat-num">∞</div><div className="hero-stat-label">Personalisation</div></div>
          <div className="hero-divider" />
          <div><div className="hero-stat-num">01</div><div className="hero-stat-label">Community</div></div>
        </div>
      </section>
      ...
      <section id="showcase" className="section media-bg" style={{ paddingBottom: 0 }}>
        ...
        {mediaLoading ? (
          ...
          <div style={{ textAlign: "center", padding: "80px 24px", border: "0.5px solid rgba(160,120,40,0.15)" }}>
            <div style={{ fontSize: 36, color: "rgba(160,120,40,0.2)", marginBottom: 16 }}>◈</div>
          ...
          <div className="media-grid" style={{ opacity: 1, transform: "none" }}>
            {mediaItems.map((item, i) => (
              <div key={item._id || i} className={`media-item ${item.featured ? "featured" : ""}`}>
                ...
                <div style={{ display: item.img ? "none" : "flex", width: "100%", height: "100%", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 8, background: "var(--surface)" }}>
                  <div style={{ fontSize: 32, color: "rgba(160,120,40,0.2)" }}>◈</div>
        ...
        <div className="featured-split">
          ...
          <div className="fade-in">
            <div className="featured-visual">
              <div style={{ fontSize: 48, color: "rgba(160,120,40,0.2)" }}>◈</div>
      ...
      <section className="section" style={{ background: "var(--dark3)", textAlign: "center" }}>
        <div className="fade-in" style={{ maxWidth: 600, margin: "0 auto" }}>
          ...
          <h2 className="section-title" style={{ marginBottom: 20 }}>This is not a newsletter.<br />It's a <em>private circle.</em></h2>
